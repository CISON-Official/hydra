...
class CertificateUpdateService:
    ...
    async def update_certificate_file(
        ...
        try:
            etag, file_size = await self.s3_client.upload_file(
                file_content, new_s3_key, new_file.content_type
            )
        except Exception as e:
            raise HTTPException(
                status_code=500, detail=f"Failed to upload new version: {str(e)}"
            )
    ...
    async def _archive_current_version(
        self, cert: Certificate, updated_by: UUID, update_reason: str, db: AsyncSession
    ):
    ...
    async def rollback_to_version(
        ...
        await self._archive_current_version(
            cert, rolled_back_by, f"Pre-rollback to v{target_version}", db
        )
    ...
    async def get_version_history(
        self, certificate_id: UUID, db: AsyncSession, limit: int = 50
    ) -> List[Dict[str, Any]]:
        ...
        stmt = (
            ...
            .order_by(CertificateVersion.version_number.desc())
            .limit(limit)
        )
        ...
        if current:
            history.insert(
                0,
                {
                    ...
                    "updated_by": (
                        str(current.last_updated_by)
                        if current.last_updated_by
                        else None
                    ),
    ...
    async def compare_versions(
        self, certificate_id: UUID, version_a: int, version_b: int, db: AsyncSession
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "version_a": {
                ...
                "updated_at": (
                    version_a_data.created_at.isoformat()
                    if hasattr(version_a_data, "created_at")
                    else version_a_data.updated_at.isoformat()
                ),
            },
            "version_b": {
                "number": version_b,
                ...
                "updated_at": (
                    version_b_data.created_at.isoformat()
                    if hasattr(version_b_data, "created_at")
                    else version_b_data.updated_at.isoformat()
                ),
            },
            "identical": version_a_data.file_hash == version_b_data.file_hash,
            "size_difference_bytes": abs(
                version_a_data.file_size_bytes - version_b_data.file_size_bytes
            ),
        }
    ...
    async def extend_expiry_with_update(
        ...
        return {
            ...
            "new_version": (
                cert.current_version if file_update_result.get("updated") else None
            ),
